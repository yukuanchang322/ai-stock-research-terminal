...
function candleSvg(series,meta=candlePeriodState){
  ...
  const labels={day:'日K',week:'週K',month:'月K'},units={day:'個交易日',week:'週',month:'月'};
  return `<div class="candle-chart"><div class="tech-chart-title"><b>${labels[meta.interval]||'日K'}</b><span>點選價位 · 左右拖曳 · 雙指縮放</span></div><div class="candle-period-tabs" role="tablist" aria-label="K線週期"><button type="button" data-candle-period="day" class="${meta.interval==='day'?'active':''}">日線</button><button type="button" data-candle-period="week" class="${meta.interval==='week'?'active':''}">週線・10年</button><button type="button" data-candle-period="month" class="${meta.interval==='month'?'active':''}">月線・10年</button></div><div class="candle-controls" aria-label="K線縮放控制"><button type="button" data-candle-action="zoom-in" aria-label="放大K線">＋</button><button type="button" data-candle-action="zoom-out" aria-label="縮小K線">－</button><button type="button" data-candle-action="reset">重設範圍</button><span>${data.length} ${units[meta.interval]||'筆'}</span></div><svg class="candle-svg" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" role="img" aria-label="${labels[meta.interval]||'日K'}，含日期與價格刻度，可縮放與左右移動">${priceTicks}<line class="axis" x1="${p}" y1="${h-p}" x2="${w-p}" y2="${h-p}"/>${candles}${p20}${p60}<line class="latest-price-line" x1="${p}" y1="${latestY}" x2="${w-p}" y2="${latestY}"/>${dateTicks}<g class="candle-crosshair" hidden><line class="crosshair-x" y1="${p}" y2="${h-p}"/><circle class="crosshair-dot" r="4"/></g><rect class="candle-hit" x="${p}" y="${p}" width="${w-2*p}" height="${h-2*p}"/></svg><div class="candle-details" aria-live="polite">${candleDetailsHtml(latest,latestChange)}</div><div class="candle-source">${meta.source||'—'} · 截至 ${meta.asOf||latest.date||'—'}</div><div class="chart-legend"><span>MA20</span><span>MA60</span></div></div>`;
}
...
function bindCandlePeriods(){
  document.querySelectorAll('[data-candle-period]').forEach(button=>button.addEventListener('click',()=>switchCandlePeriod(button.dataset.candlePeriod)));
}
async function switchCandlePeriod(interval){
...
function render(d){
  ...
  const brief=d.decision_brief||{}, scenarios=brief.scenarios||d.valuation.scenarios||[];
...
const ex=d.expectation_gap||{};
  ...
  if($('pipelineView')) $('pipelineView').innerHTML=`<div class="pipeline-stance"><b>${rp.stance||'—'}</b><span>Research Score ${rp.research_score??'—'}</span></div><p>${rp.investment_view||'目前資料不足以形成完整研究結論。'}</p><div class="evidence-mini"><span>結論強度 ${brief.conclusion_strength||'—'}</span><span>機率加權價值 ${fmt0(brief.expected_target)}</span><span>相對現價 ${pct(brief.expected_upside_pct)}</span></div><p class="analysis">${brief.variant_perception||'目前無法判斷市場預期差。'}</p>${(brief.quality_gates||[]).length?`<div class="valuation-alert"><b>結論降級原因</b><br>${brief.quality_gates.join('；')}</div>`:''}<div class="evidence-mini"><span>Fact ${rp.evidence_counts?.facts??0}</span><span>Derived ${rp.evidence_counts?.derived_facts??0}</span><span>Estimate ${rp.evidence_counts?.estimates??0}</span></div>`;
  ...
  $('valuationBody').innerHTML=scenarios.length?scenarios.map(x=>`<tr><td>${x.name}</td><td>${x.probability_pct==null?'—':`${fmt(x.probability_pct,0)}%`}</td><td>${fmt(x.eps,2)}</td><td>${fmt(x.pe,1)}x</td><td><b>${fmt0(x.target)}</b></td><td>${pct(x.upside_pct)}</td></tr>`).join(''):'<tr><td colspan="6">估值資料不足</td></tr>';
  $('assumptions').innerHTML=`<div class="assumption-row"><b>EPS Basis</b><span>${d.valuation.eps_basis||'—'}</span></div><div class="assumption-row"><b>PE Basis</b><span>${d.valuation.pe_basis||'—'}</span></div><div class="assumption-row"><b>機率加權期望值</b><span>${fmt0(brief.expected_target)}（${pct(brief.expected_upside_pct)}）</span></div><div class="assumption-row"><b>資料完整度 / 估值信心</b><span>${d.confidence?.data_completeness??0}/100 · ${d.valuation.confidence||0}/100</span></div><div class="assumption-row"><b>模型原則</b><span>${brief.methodology||'歷史回歸、Forward EPS、法人目標與市場隱含估值分開呈現，不合併成單一保證價。'}</span></div>`; $('peBand').innerHTML=`歷史 PER：P25 <b>${fmt(d.per?.pe_p25,1)}x</b> · Median <b>${fmt(d.per?.pe_median,1)}x</b> · P75 <b>${fmt(d.per?.pe_p75,1)}x</b>${d.valuation.regime_shift_warning?'<div class="valuation-alert">目前市場 PER 已明顯脫離歷史區間，歷史回歸價僅供風險壓力測試。</div>':''}`;
