...
function render(d){
  ...
  const evidenceHtml=`<div class="evidence-matrix"><h4>Multi-Source Evidence Matrix</h4><div class="evidence-stats"><div><span>Evidence</span><b>${esum.usable??0}</b></div><div><span>官方/驗證</span><b>${esum.official_or_verified??0}</b></div><div><span>Fact</span><b>${esum.facts??0}</b></div><div><span>Derived</span><b>${esum.derived_facts??0}</b></div><div><span>衝突</span><b>${esum.conflicts??0}</b></div><div><span>Evidence Score</span><b>${esum.evidence_score??0}</b></div></div>${(eg.conflicts||[]).length?`<div class="evidence-conflicts"><b>來源衝突</b>${eg.conflicts.slice(0,4).map(c=>`<small>${c.metric} ${c.period||''} · spread ${c.spread_pct}%</small>`).join('')}</div>`:'<small class="evidence-ok">目前核心證據未偵測到重大數值衝突。</small>'}<a class="diagnostic-link" href="/api/evidence/${encodeURIComponent(d.ticker)}" target="_blank" rel="noopener noreferrer">查看完整 Evidence JSON</a></div>`;
