...
(() => {
  ...
  window.fetch=async function(input,init){
    ...
    try{
      ...
      return new Response(JSON.stringify(recovered),{status:200,headers:{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store','X-AI-Stock-Recovery':'1','X-AI-Stock-Version':VERSION}});
    }catch(err){
      const record=await read(ticker);
      if(!record)throw err;
      const recovered=clone(record.data);
  ...
  function syncVisibleVersion(){
    ...
    document.querySelectorAll('.kpi small').forEach(el=>{
      if(/^V5\.2\.15$/.test((el.textContent||'').trim())) el.textContent=`V${VERSION}`;
    });
  ...
  window.addEventListener('load',()=>setTimeout(syncVisibleVersion,250));
