...
def _snapshot_from_diag(diag: dict) -> dict | None:
    ...
    valid.sort(key=lambda x: (x.get("fiscal_year") or 0, x.get("fiscal_quarter") or 0, x.get("completeness") or 0), reverse=True)
    out = dict(valid[0]); out["official"] = True
    return out


def _diag_reason(diag: dict) -> str:
    if diag.get("company_mops_error"): return "MOPS公司財報查詢失敗"
    ...
    if diag.get("expected_period"): return f"官方尚未取得 {diag.get('expected_period')} 可解析財報"
...
def _write_source_status(d: dict, name: str, *, status: str, as_of: str | None, reason: str | None = None):
    ...
    if not isinstance(rows, list): rows = []; d["source_status"] = rows
    row = next((x for x in rows if isinstance(x, dict) and x.get("name") == name), None)
    if row is None:
        row = {"name": name, "dataset": name, "scheduled_update": "官方公告後更新"}; rows.append(row)
    row["status"] = status; row["as_of"] = as_of
    if reason:
        row["reason"] = reason
        if not as_of: row["as_of"] = f"缺資料｜{reason}"


async def _financial_recovery(ticker: str, d: dict) -> dict:
    if _financial_present(d): return d
    ...
    try:
        ...
        snap = None; pd["financial_v5100_primary"] = {"status": "error", "error": f"{type(e).__name__}: {str(e)[:160]}"}
    diag = None
    ...
    if not isinstance(snap, dict) or not snap.get("official") or not any(snap.get(k) is not None for k in ("ytd_eps", "revenue_ytd", "gross_profit_ytd", "net_income_ytd")):
        ...
        return d
    try: eps_stack = await asyncio.wait_for(server.build_eps_stack(ticker, [], snap, {}), timeout=12)
    except Exception: eps_stack = {"ytd_eps": snap.get("ytd_eps"), "quarter_period": snap.get("period"), "source": snap.get("source")}
    try: integrity = server.assess_financial_integrity(snap, eps_stack, date.today())
    except Exception: integrity = {}
    fin = {"statement_date": snap.get("statement_date") or snap.get("period"), "period": snap.get("period"), "source": snap.get("source"), "official": True, "partial": False, "ytd_eps": snap.get("ytd_eps"), "quarter_eps": eps_stack.get("quarter_eps"), "ttm_eps": eps_stack.get("ttm_eps"), "revenue": snap.get("revenue_ytd"), "gross_profit": snap.get("gross_profit_ytd"), "operating_income": snap.get("operating_income_ytd"), "net_income": snap.get("net_income_ytd")}
    rev = fin.get("revenue")
    ...
    d["official_financial"] = snap; d["financial"] = fin; d["eps_stack"] = eps_stack; d["financial_integrity"] = integrity
...
async def build_stock_v5100(ticker: str, force_refresh: bool = False):
    ...
    if not (isinstance(rev, dict) and rev.get("series")):
        try:
            ...
            if isinstance(stable.get("revenue"), dict) and stable["revenue"].get("series"): d["revenue"] = stable["revenue"]
...
async def api_stock_v5100(ticker: str, refresh: int = 0): return await build_stock_v5100(ticker.strip(), force_refresh=bool(refresh))

@app.get("/api/v5100/stock/{ticker}")
async def api_v5100(ticker: str, refresh: int = 0): return await build_stock_v5100(ticker.strip(), force_refresh=bool(refresh))

@app.get("/api/v5100/financial-diagnostics/{ticker}")
async def financial_diag_v5100(ticker: str): return await server.diagnose_official_financial_sources(ticker.strip())

@app.get("/health")
async def health_v5100(): return JSONResponse({"status": "ok", "version": VERSION, "mode": "integrated-core-financial-version-integrity"}, headers={"Cache-Control": "no-store"})
...
async def root_v5100():
    ...
    if "v5100_hotfix.js" not in text:
        text = text.replace("</body>", f'<script src="v5100_hotfix.js?v={VERSION}"></script></body>')
