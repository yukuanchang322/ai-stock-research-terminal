...
def write_professional_pdf(data:dict[str,Any],out:Path,root:Path,version:str):
    ...
    sc=data.get("scores") or {}; fin=data.get("financial") or {}; eps=data.get("eps_stack") or {}; rev=data.get("revenue") or {}; tech=data.get("technical") or {}; flow=data.get("flow") or {}; val=data.get("valuation") or {}; research=data.get("research") or {}; pipe=data.get("research_pipeline") or {}; conf=data.get("confidence") or {}; per=data.get("per") or {}; brief=data.get("decision_brief") or {}
    ...
    story += [_table(metrics,font,[42*mm]*4),Spacer(1,8*mm),Paragraph("投資結論",h1),Paragraph(_esc(data.get("thesis") or pipe.get("investment_view") or "資料不足，暫不形成結論。"),call),Spacer(1,3*mm)]
    if brief:
        story += [_table([["結論強度","機率加權價值","相對現價","研究預期差"],[brief.get("conclusion_strength"),_fmt(brief.get("expected_target"),0," 元"),_pct(brief.get("expected_upside_pct")),brief.get("variant_perception")]],font,[27*mm,34*mm,30*mm,77*mm],size=8)]
        if brief.get("quality_gates"):
            story += [Paragraph("結論降級原因："+"；".join(_esc(x) for x in brief.get("quality_gates") or []),warn)]
    ...
    scenario_rows=brief.get("scenarios") or val.get("scenarios") or []
    scen=[["情境","機率","EPS","合理PE","模型價格","相對現價"]]+[[x.get("name"),_pct(x.get("probability_pct"),0),_fmt(x.get("eps"),2),_fmt(x.get("pe"),1,"x"),_fmt(x.get("target"),0),_pct(x.get("upside_pct"))] for x in scenario_rows]
    ...
    story += [Paragraph("05｜估值、法人預期與市場隱含假設",h1),_table(rec,font,[39*mm,31*mm,48*mm,27*mm,32*mm]),Spacer(1,3*mm),Paragraph(_esc(val.get("valuation_warning") or "估值模型對EPS與倍數高度敏感。"),warn),Paragraph("情境估值與機率加權期望值",h2),_table(scen,font,[26*mm,24*mm,27*mm,27*mm,34*mm,32*mm],right=1),Spacer(1,5*mm),Paragraph(f"機率加權模型價格 {_fmt(brief.get('expected_target'),0,' 元')}｜相對現價 {_pct(brief.get('expected_upside_pct'))}。{_esc(brief.get('methodology') or '')}",call),Paragraph("EPS × PE 敏感度矩陣（元）",h2),_table(sens,font,[44*mm]+[42*mm]*len(pv),right=1),Paragraph(f"歷史PER分位：P25 {_fmt(per.get('pe_p25'),1)}x｜中位數 {_fmt(per.get('pe_median'),1)}x｜P75 {_fmt(per.get('pe_p75'),1)}x｜樣本 {per.get('sample_count','—')} 日。",small),PageBreak()]
    # Public research
